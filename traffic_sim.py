...
class TrafficSimulation:
    def __init__(self, width: int = 1200, height: int = 900):
        ...
        self.spawn_rate = 2.0      # segundos entre spawns (aprox)
        self.last_spawn_time = 0   # un solo reloj global de spawn
        self.current_time = 0
    ...
    def draw_traffic_light(self, x: int, y: int, state: LightState, horizontal: bool = False) -> None:
        ...
        if not horizontal:
            # Semáforo VERTICAL (como los que ven N y S)
            ...
            # Semáforo HORIZONTAL (como los que ven E y W)
    ...
    def draw_car(self, car: Car, center_x: int, center_y: int) -> None:
        # Tamaño total del cruce (el mismo que usas en draw_intersection)
        intersection_size = 800

        # s = distancia recorrida a lo largo del carril (0 a 800 aprox)
        s = car.position

        if car.direction == Direction.NORTH:
            # De abajo hacia arriba, por el carril derecho de la línea amarilla
            x = center_x + 425
            y = center_y + intersection_size - s  # empieza abajo y sube
            width, height = 20, 30

        elif car.direction == Direction.SOUTH:
            # De arriba hacia abajo, por el carril izquierdo
            x = center_x + 375
            y = center_y + s  # empieza arriba y baja
            width, height = 20, 30

        elif car.direction == Direction.EAST:
            # De izquierda a derecha, por el carril de abajo
            x = center_x + s  # empieza a la izquierda y avanza a la derecha
            y = center_y + 425
            width, height = 30, 20

        else:  # Direction.WEST
            # De derecha a izquierda, por el carril de arriba
            x = center_x + intersection_size - s  # empieza a la derecha y va a la izquierda
            y = center_y + 375
            width, height = 30, 20
    ...
    def draw_intersection(self) -> None:
        ...
        # Bordes de las calles (coinciden con los rectángulos que ya dibujas)
        road_v_left = center_x + 350      # vertical: x de la izquierda
        road_v_right = center_x + 450     # vertical: x de la derecha
        road_h_top = center_y + 350       # horizontal: y superior
        road_h_bottom = center_y + 450    # horizontal: y inferior
        ...
        # NORTH: coches que vienen desde abajo y van hacia arriba
        # Semáforo vertical a la DERECHA de la calle, fuera del carril
        self.draw_traffic_light(
            road_v_right + 10,  # un poco fuera de la calle
            road_h_bottom - 95, # pegado al cruce pero antes de la intersección
            self.traffic_controller.get_light_state(Direction.EAST),
            horizontal=False
        )

        # SOUTH: coches que vienen desde arriba y van hacia abajo
        # Semáforo vertical a la IZQUIERDA de la calle, fuera del carril
        self.draw_traffic_light(
            road_v_left - 34,   # 24 de ancho + margen
            road_h_top + 5,
            self.traffic_controller.get_light_state(Direction.WEST),
            horizontal=False
        )

        # EAST: coches que vienen desde la izquierda y van a la derecha
        # Semáforo horizontal DEBAJO de la calle, fuera del carril
        self.draw_traffic_light(
            road_v_left + 5,
            road_h_bottom + 10,
        ...
        # WEST: coches que vienen desde la derecha y van a la izquierda
        # Semáforo horizontal ENCIMA de la calle, fuera del carril
        self.draw_traffic_light(
            road_v_right - 95,  # 90 de ancho + margen
            road_h_top - 34,    # 24 de alto + margen
    ...
    def draw_control_panel(self) -> None:
        ...
        counts = {
            "North": self.car_manager.get_queue_count(Direction.SOUTH),   # lo que ves arriba
            "South": self.car_manager.get_queue_count(Direction.NORTH),   # lo que ves abajo
            "East":  self.car_manager.get_queue_count(Direction.WEST),    # lo que ves a la derecha
            "West":  self.car_manager.get_queue_count(Direction.EAST),    # lo que ves a la izquierda
        }

        for name, count in counts.items():
            queue_text = self.font_small.render(f"{name}: {count}", True, (50, 50, 50))
